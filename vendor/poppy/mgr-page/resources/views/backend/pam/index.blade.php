...
@section('backend-main')
    <div class="layui-card-header">
        ...
        <div class="pull-right">
            @can('create', config('poppy.core.rbac.role'))
                <a href="{{route_url('py-mgr-page:backend.pam.establish', [], ['type' => $type,])}}"
                        ...
                        <td>
                            @if ($_pam->can('disable', $item))
                                <a class="J_iframe J_tooltip" title="当前启用, 点击禁用"
                                   href="{{route_url('py-mgr-page:backend.pam.disable',[$item->id])}}">
                                    <i class="fa fa-unlock text-success"></i>
                                </a>
                            @endif
                            @if ($_pam->can('enable', $item))
                                <a class="J_iframe J_tooltip" title="当前禁用, 点击启用"
                                   href="{{route_url('py-mgr-page:backend.pam.enable',[$item->id])}}">
                                    <i class="fa fa-lock text-danger"></i>
                                </a>
                            @endif
                            <a title="编辑[{{$item->username}}]" class="J_iframe J_tooltip"
                               href="{{route('py-mgr-page:backend.pam.establish', [$item->id])}}">
                                <i class="fa fa-edit"></i>
                            </a>
                            <a title="修改密码" class="J_iframe J_tooltip"
                               href="{{route('py-mgr-page:backend.pam.password', [$item->id])}}">
                                <i class="fa fa-key"></i>
                            </a>
                        </td>
                    </tr>
