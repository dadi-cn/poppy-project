@if (data_get($definition, 'sign_token', true))
    <p class="layui-form-item mt10">
        ...
        @if (Route::has('py-mgr-page:develop.cp.set_token'))
            <a href="{!! route_url('py-mgr-page:develop.cp.set_token', null, ['type'=> $guard]) !!}"
               data-title="设置 Token {!! $guard !!}"
               class="J_iframe pull-right layui-btn layui-btn-sm">设置 Token</a>
        @endif
        @if (Route::has('py-mgr-page:develop.cp.api_login'))
            <a href="{!! route_url('py-mgr-page:develop.cp.api_login', null, ['type'=> $guard]) !!}"
               data-title="登录 {!! $guard !!}"
